...
namespace litehtml
{
    ...
    struct position
    {
        ...
        void move_to(int in_x, int in_y)
        {
            this->x = in_x;
            this->y = in_y;
        }
        ...
        bool is_point_inside(int in_x, int in_y) const
        {
            if(in_x >= left() && in_x <= right() && in_y >= top() && in_y <= bottom())
            {
                return true;
            }
            return false;
    ...
    typedef std::map<std::string, font_item> fonts_map;
    ...
    struct containing_block_context
    {
        ...
        struct typed_int
        {
            ...
            typed_int(const typed_int& val) = default;
